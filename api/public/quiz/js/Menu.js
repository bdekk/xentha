...
Quiz.Menu.prototype = {
  ...
  create: function() {
    ...
    playersText = this.game.add.text(100, this.game.world.centerY, "Players", {fill: '#fff'});
  ...
  onStart: function() {
    ...
    var transition = this.game.add.tween(this.game.world).to( { alpha: 0 }, 1000, "Linear", true);
  ...
  xentha: function() {
    ...
    //   console.log('player left.. ');
      var index = me.connected.map(function(player) {return player.id; }).indexOf(data.player.id);
  ...
  update: function() {
    ...
    for(var i =0; i < this.connected.length; i++) {
      ...
      this.game.add.text(100, this.game.world.centerY + ((i + 1) * 50), name, {fill: this.connected[i].color});
    }
