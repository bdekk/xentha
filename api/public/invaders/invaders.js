...
var addSpriteToData = function(data) {
...
XENTHA.playerLeft = function(data) {
  var index = -1;
  for(var i =0; i < data.players.length; i++) {
    if(data.players[i].id == data.id) { index = i; };
  }
  if(index != -1) {
    data.players.splice(index);
  }
}
...
XENTHA.movePlayerLeft = function(data) {
  for(var i =0; i < data.players.length; i++) {
    data.players[i].player.body.velocity.x = -200;
  }
