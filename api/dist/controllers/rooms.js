...
methods.join = function (req, res, next) {
    if (!req.params.roomId && !req.body.userId)
        return;
    //join sockets is done through socket io (controllers/io/rooms.join)
    Room.findOne({ where: { roomCode: req.params.roomCode } }).then(function (room) {
...
methods.getOne = function (req, res, next) {
    Room.find({ where: { roomCode: req.params.roomCode } }).then(function (room) {
...
methods.changeState = function (req, res, next) {
    Room.update({ status: req.body.status }, { where: { roomCode: req.params.roomCode } }).then(function (result) {
        if (result && result[0] !== 0) {
            return res.send({ roomCode: req.params.roomCode, status: req.body.status });
        }
...
methods.serverLobby = function (req, res, next) {
    // //TODO: spread.
    // Room.findOne({where: {roomCode: req.params.roomCode}}).then(function foundRoom(room) {
    // 	if(room) {
