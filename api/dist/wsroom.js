...
exports.create = function (socket, room, opts) {
    ...
    if (exports.Debug)
        console.log(socket.id + ": Creating Room: " + room);
    if (this.exists(room)) {
        console.log('! overriding room with id ' + room);
    }
    ;
...
exports.set = function (socket, id, data) {
    if (exports.Debug)
        console.log(socket.id + ": Creating variable: " + id + " with data: " + data);
...
exports.get = function (socket, variable, content) {
    ...
    if (variable == "host")
        return this.rooms[socket.room].host;
    if (variable == "sockets")
        return this.rooms[socket.room].sockets;
    if (variable == "hostAndSockets")
        return this.rooms[socket.room].sockets.concat(this.rooms[socket.room].host);
...
exports.leave = function (socket) {
    ...
    if (exports.Debug)
        console.log(socket.id + ": Leaving room: " + socket.room);
    var i = this.rooms[socket.room].sockets.indexOf(socket.id);
    console.log(i);
    if (i != -1)
        this.rooms[socket.room].sockets.splice(i, 1);
